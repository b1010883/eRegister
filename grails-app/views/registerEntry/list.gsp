			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="date" title="${message(code: 'registerEntry.date.label', default: 'Date')}" />
					
						<th><g:message code="registerEntry.registrationSheet.label" default="Registration Sheet" /></th>
				...
				<g:each in="${registerEntryInstanceList}" status="i" var="registerEntryInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${registerEntryInstance.id}">${fieldValue(bean: registerEntryInstance, field: "date")}</g:link></td>
